...
### Fixes
* Web: a chat entry's HTML is no longer rebuilt from scratch on a render where it could not have changed — a cheap per-entry signature (message id/timestamp/reply/sender fields, or a log entry's type/timestamp/subject-node fields, plus the active filter) is checked first, and the underlying string-render function only runs on a mismatch; a render where only the Log tab needs new content (nothing message- or display-related changed) also skips rebuilding every channel tab's descriptor and factory closure, reusing the previous tick's
* Web: a chat channel tab renders only its newest 500 messages by default, with a "show older" control that loads its full seven-day window on demand — amending issue #796's "render the entire window" to "load the entire window, render on demand" now that the window can be thousands of messages deep; expanding one tab rebuilds only that tab in place, keeping the reader's scroll position anchored and leaving every other tab untouched
* Web: the shared relative-time ticker that keeps "last seen"/"last update" fields counting up now scans only the currently-relevant DOM (the node table body, the visible chat panel, the map, open overlays, the header) once per second instead of the whole document — a busy instance's thousands of stamped rows no longer cost a full-document sweep every second; a chat tab that was hidden snaps its own fields immediately on activation rather than waiting for the next tick
* Web: chat channel tabs build their content lazily — only the tab the reader currently has open is rendered; switching tabs builds the newly-active one on first visit and never rebuilds it again — instead of every channel's DOM subtree being rebuilt on every chat render; the message→tab-header flash map is now derived from the chat model directly so a hidden channel's tab header still flashes even though its content is never built (SPEC VF3)
* Web: the background chat-history backfill (issue #802) no longer rebuilds the whole chat log after every streamed page — pages merge immediately but the repaint coalesces onto the same idle-time mechanism the bulk-collection backfill (issue #832) already used, and that mechanism's own repaint is now routed to only the render stages the touched collections feed instead of always doing a full repaint
...
* Web: the map's neighbor-connection lines are now diffed across renders — only a direction whose endpoints, colour, SNR, or names actually changed is torn down and recreated, instead of every neighbor line being cleared and redrawn on every map render; their tooltips also build their HTML lazily on hover instead of upfront for every line. Trace lines (not diffed) now draw on a canvas renderer when available, since they carry no click-to-element anchor and canvas avoids one SVG element per segment
* Web: a live-driven refresh's node-derived state (display name, position, distance, telemetry) now re-aggregates only the nodes this tick's delta actually touched, instead of every node in the collection — a node not touched this tick keeps its exact previous object, unallocated. The position/telemetry/neighbor/trace/waypoint accumulators now merge and window-evict old rows in a single pass instead of two, with the eviction check itself throttled to once/60s when nothing new arrived
* Web: a live-driven dashboard refresh now routes each tick to only the render stages the collections that actually changed touch, instead of unconditionally rebuilding the node table, the whole map, and every chat tab on every SSE ping — a messages-only ping no longer re-clears and redraws the map, and a nodes/positions/telemetry-only ping no longer rebuilds the chat log; chat message hydration is likewise limited to the incoming delta rows instead of the full retained window (SPEC PS-perf)
